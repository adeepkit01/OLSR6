...
#ifndef OLSR6_AGENT_IMPL_H
#define OLSR6_AGENT_IMPL_H
...
class Olsr6MprTestCase;

namespace ns3 {
namespace olsr6 {

///
/// \defgroup olsr6 OLSR6 Routing
/// This section documents the API of the ns-3 OLSR6 module. For a generic
/// functional description, please refer to the ns-3 manual.

/// \ingroup olsr6
/// An %OLSR6's routing table entry.
struct RoutingTableEntry
{
  Ipv6Address destAddr; //!< Address of the destination node.
  Ipv6Address nextAddr; //!< Address of the next hop.
...
/// \ingroup olsr6
///
/// \brief OLSR6 routing protocol for IPv4
///
class RoutingProtocol : public Ipv6RoutingProtocol
{
public:
  friend class ::Olsr6MprTestCase;
  ...
  /**
   * \brief Set the OLSR6 main address to the first address on the indicated interface.
   *
  ...
  /**
   * Return the list of routing table entries discovered by OLSR6
   */
...
private:
  ...
  Ptr<Ipv6StaticRouting> m_routingTableAssociation; //!< Associations from an Ipv4StaticRouting instance
...
public:
   ...
   */
  void AddHostNetworkAssociation (Ipv6Address networkAddr, Ipv6Prefix netmask);

  /**
   * \brief Removes the specified (networkAddr, netmask) tuple from the list of
   * local HNA associations to be sent by the node via HNA messages.
   * If this tuple does not exist, nothing is done (see "Olsr6State::EraseAssociation()").
   *
   * \param networkAddr The network address.
   * \param netmask The network mask.
   */
  void RemoveHostNetworkAssociation (Ipv6Address networkAddr, Ipv6Prefix netmask);

  /**
   * \brief Associates the specified Ipv6StaticRouting routing table
   *         to the OLSR6 routing protocol. Entries from this associated
   *         routing table that use non-olsr6 outgoing interfaces are added
   ...
   */
  void SetRoutingTableAssociation (Ptr<Ipv6StaticRouting> routingTable);

  /**
   * \brief Returns the internal OLSR6_MCAST_GLOBALHNA table
   * \returns the internal HNA table
   */
  Ptr<const Ipv6StaticRouting> GetRoutingTableAssociation () const;
  virtual void PrintRoutingTable (Ptr<OutputStreamWrapper> stream) const;
protected:
  virtual void DoInitialize (void);
private:
  std::map<Ipv6Address, RoutingTableEntry> m_table; //!< Data structure for the routing table.

  Ptr<Ipv6StaticRouting> m_hnaRoutingTable; //!< Routing table for HNA routes
  ...
  Olsr6State m_state;  //!< Internal state with all needed data structs.
  Ptr<Ipv6> m_ipv6;   //!< IPv4 object the routing is linked to.
   ...
   */
  void AddEntry (const Ipv6Address &dest,
                 const Ipv6Address &next,
                 uint32_t interface,
                 uint32_t distance);
  /**
   * \brief Adds a new entry into the routing table.
  ...
  void AddEntry (const Ipv6Address &dest,
                 const Ipv6Address &next,
                 const Ipv6Address &interfaceAddress,
                 uint32_t distance);
   ...
   */
  bool Lookup (const Ipv6Address &dest,
  ...
  virtual void NotifyAddRoute (Ipv6Address dst, Ipv6Prefix mask, Ipv6Address nextHop, uint32_t interface, Ipv6Address prefixToUse);
  virtual void NotifyRemoveRoute (Ipv6Address dst, Ipv6Prefix mask, Ipv6Address nextHop, uint32_t interface, Ipv6Address prefixToUse);
  ...
  /**
   * Receive an OLSR6 message.
   * \param socket The receiving socket.
   */
  void RecvOlsr6 (Ptr<Socket> socket);
  ...
  /**
   *  \brief Tests whether or not the specified route uses a non-OLSR6 outgoing interface.
   *  \param route The route to be tested.
   *  \returns True if the outgoing interface of the specified route is a non-OLSR6 interface, false otherwise.
   */
  bool UsesNonOlsr6OutgoingInterface (const Ipv6RoutingTableEntry &route);
   ...
   *
   * The task of actually removing the tuple is left to the OLSR6 agent.
   *
   * \param address The address of the tuple.
   * \param sequenceNumber The sequence number of the tuple.
   */
  void DupTupleTimerExpire (Ipv6Address address, uint16_t sequenceNumber);
   ...
   */
  void LinkTupleTimerExpire (Ipv6Address neighborIfaceAddr);
   ...
   */
  void Nb2hopTupleTimerExpire (Ipv6Address neighborMainAddr, Ipv6Address twoHopNeighborAddr);
   ...
   * The task of actually removing the tuple is left to the OLSR6 agent.
   *
   * \param mainAddr The tuple IPv6 address.
   */
  void MprSelTupleTimerExpire (Ipv6Address mainAddr);
   ...
   */
  void TopologyTupleTimerExpire (Ipv6Address destAddr, Ipv6Address lastAddr);
   ...
   */
  void IfaceAssocTupleTimerExpire (Ipv6Address ifaceAddr);
   ...
   * \param networkAddr The network address.
   * \param netmask  The network prefix.
   */
  void AssociationTupleTimerExpire (Ipv6Address gatewayAddr, Ipv6Address networkAddr, Ipv6Prefix netmask);
  ...
  olsr6::MessageList m_queuedMessages;
  Timer m_queuedMessagesTimer; //!< timer for throttling outgoing messages

  /**
   * \brief OLSR6's default forwarding algorithm.
   *
   * See \RFC{3626} for details.
   *
   * \param olsr6Message The %OLSR6 message which must be forwarded.
   ...
   * \param senderAddress The sender IPv6 address.
   */
  void ForwardDefault (olsr6::MessageHeader olsr6Message,
                       DuplicateTuple *duplicated,
                       const Ipv6Address &localIface,
                       const Ipv6Address &senderAddress);

  /**
   * \brief Enques an %OLSR6 message which will be sent with a delay of (0, delay].
   *
   * This buffering system is used in order to piggyback several %OLSR6 messages in
   * a same %OLSR6 packet.
   *
   * \param message the %OLSR6 message which must be sent.
   * \param delay maximum delay the %OLSR6 message is going to be buffered.
   */
  void QueueMessage (const olsr6::MessageHeader &message, Time delay);

  /**
   * \brief Creates as many %OLSR6 packets as needed in order to send all buffered
   * %OLSR6 messages.
   *
   * Maximum number of messages which can be contained in an %OLSR6 packet is
   * dictated by OLSR6_MAX_MSGS constant.
   */
  void SendQueuedMessages ();

  /**
   * \brief Creates a new %OLSR6 HELLO message which is buffered for being sent later on.
   */
  void SendHello ();

  /**
   * \brief Creates a new %OLSR6 TC message which is buffered for being sent later on.
   */
  void SendTc ();

  /**
   * \brief Creates a new %OLSR6 MID message which is buffered for being sent later on.
   */
  void SendMid ();

  /**
   * \brief Creates a new %OLSR6 HNA message which is buffered for being sent later on.
   */
  void SendHna ();
   ...
   *
   * \param msg the %OLSR6 message which contains the HELLO message.
   ...
   */
  void ProcessHello (const olsr6::MessageHeader &msg,
   ...
   *
   * \param msg The %OLSR6 message which contains the TC message.
   * \param senderIface The address of the interface where the message was sent from.
   *
   */
  void ProcessTc (const olsr6::MessageHeader &msg,
                  const Ipv6Address &senderIface);
   ...
   *
   * \param msg the %OLSR6 message which contains the MID message.
   * \param senderIface the address of the interface where the message was sent from.
   */
  void ProcessMid (const olsr6::MessageHeader &msg,
                   const Ipv6Address &senderIface);
   ...
   *
   * \param msg the %OLSR6 message which contains the HNA message.
   * \param senderIface the address of the interface where the message was sent from.
   *
   */
  void ProcessHna (const olsr6::MessageHeader &msg,
                   const Ipv6Address &senderIface);
   ...
   */
  void LinkSensing (const olsr6::MessageHeader &msg,
   ...
   * \param hello The received HELLO sub-message.
   */
  void PopulateNeighborSet (const olsr6::MessageHeader &msg,
                            const olsr6::MessageHeader::Hello &hello);
   ...
   * \param hello The received HELLO sub-message.
   */
  void PopulateTwoHopNeighborSet (const olsr6::MessageHeader &msg,
                                  const olsr6::MessageHeader::Hello &hello);
   ...
   * \param hello The received HELLO sub-message.
   */
  void PopulateMprSelectorSet (const olsr6::MessageHeader &msg,
                               const olsr6::MessageHeader::Hello &hello);

  int Degree (NeighborTuple const &tuple);
  /// Check that address is one of my interfaces
  bool IsMyOwnAddress (const Ipv6Address & a) const;

  Ipv6Address m_mainAddress; //!< the node main address.

  // One socket per interface, each bound to that interface's address
  // (reason: for OLSR6 Link Sensing we need to know on which interface
  // HELLO messages arrive)
  std::map< Ptr<Socket>, Ipv6InterfaceAddress > m_socketAddresses; //!< Container of sockets and the interfaces they are opened onto.
